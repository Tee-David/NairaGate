...
type KorapayEnvelope = {
  status: boolean;
  message?: string;
  data?: unknown;
};

export type KorapayProviderOptions = {
  secretKey: string;
  fetch?: FetchLike;
  baseUrl?: string;
  timeoutMs?: number;
...
function parseEnvelope(value: unknown): KorapayEnvelope | null {
...
export class KorapayProvider implements BankProvider {
  ...
  constructor(options: KorapayProviderOptions) {
    if (!options.secretKey?.trim()) {
      throw new NairaGateError(
        "INVALID_INPUT",
        "A Korapay secret key is required.",
        { provider: this.name },
      );
    }
  ...
  async listBanks(): Promise<Bank[]> {
    const params = new URLSearchParams({ countryCode: "NG" });
    const payload = await this.request(
      { path: `/misc/banks?${params.toString()}`, method: "GET" },
      "banks",
    );
    if (!Array.isArray(payload.data)) {
      throw new NairaGateError(
        "PROVIDER_ERROR",
        "Korapay returned an invalid bank response.",
        { provider: this.name },
      );
    }
  ...
  async resolveAccount(
    ...
    if (!account) {
      throw new NairaGateError(
        "PROVIDER_ERROR",
        "Korapay returned an incomplete account response.",
        { provider: this.name },
      );
    }
    return {
  ...
  private async request(
    ...
    try {
      ...
      throw new NairaGateError("NETWORK_ERROR", "Could not reach Korapay.", {
        provider: this.name,
        cause,
      });
    }
    ...
    try {
      ...
      throw new NairaGateError(
        "PROVIDER_ERROR",
        "Korapay returned an invalid response.",
        ...
        response.status === 401 || response.status === 403
          ...
          : response.status === 429
            ...
            : operation === "account" &&
              ...
              ? "ACCOUNT_NOT_FOUND"
              : "PROVIDER_ERROR";
      throw new NairaGateError(code, "Korapay request failed.", {
        provider: this.name,
