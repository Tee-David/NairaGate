...
describe("PaystackProvider", () => {
  ...
  it("resolves and normalizes an account", async () => {
    ...
    const provider = new PaystackProvider({ secretKey: "sk_test_example", fetch: fetcher as typeof fetch });
    await expect(provider.resolveAccount({ accountNumber: "0123456789", bankCode: "058" })).resolves.toEqual({
  ...
  it("rejects invalid input before a network request", async () => {
    const fetcher = vi.fn();
    const provider = new PaystackProvider({ secretKey: "sk_test_example", fetch: fetcher as typeof fetch });
    await expect(provider.resolveAccount({ accountNumber: "123", bankCode: "058" })).rejects.toMatchObject({ code: "INVALID_INPUT" });
  ...
  it.each([
    ...
    const fetcher = vi.fn(async () => jsonResponse({ status: false, message: "request failed" }, status));
    const provider = new PaystackProvider({ secretKey: "sk_test_example", fetch: fetcher as typeof fetch });
    await expect(provider.listBanks()).rejects.toMatchObject({ code, status, provider: "paystack" });
  });

  it("normalizes malformed provider responses", async () => {
    const fetcher = vi.fn(async () => new Response("not-json", { status: 502 }));
    const provider = new PaystackProvider({ secretKey: "sk_test_example", fetch: fetcher as typeof fetch });
    await expect(provider.listBanks()).rejects.toMatchObject({ code: "PROVIDER_ERROR", status: 502 });
  });
  ...
  it("normalizes network failures without leaking the secret", async () => {
    ...
    const provider = new PaystackProvider({ secretKey: "sk_test_example", fetch: fetcher as unknown as typeof fetch });
    try {
  ...
  it("rejects incomplete successful account payloads", async () => {
    const fetcher = vi.fn(async () => jsonResponse({ status: true, message: "ok", data: { account_number: "0123456789" } }));
    const provider = new PaystackProvider({ secretKey: "sk_test_example", fetch: fetcher as typeof fetch });
    await expect(provider.resolveAccount({ accountNumber: "0123456789", bankCode: "058" })).rejects.toMatchObject({ code: "PROVIDER_ERROR" });
  });
});
